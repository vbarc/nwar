...
static VkVertexInputBindingDescription getBindingDescription() {
    ...
    bindingDescription.stride = sizeof(NwVertex);
...
static std::array<VkVertexInputAttributeDescription, 3> getAttributeDescriptions() {
    ...
    attributeDescriptions[0].offset = offsetof(NwVertex, position);

    attributeDescriptions[1].binding = 0;
    attributeDescriptions[1].location = 1;
    attributeDescriptions[1].format = VK_FORMAT_R32G32B32_SFLOAT;
    attributeDescriptions[1].offset = offsetof(NwVertex, normal);

    attributeDescriptions[2].binding = 0;
    attributeDescriptions[2].location = 2;
    attributeDescriptions[2].format = VK_FORMAT_R32G32_SFLOAT;
    attributeDescriptions[2].offset = offsetof(NwVertex, uv);
...
private:
    ...
    void loadModel() {
        ...
        */
        NwTerrainGeometry terrainGeometry;
        mVertices = terrainGeometry.vertices();
        mIndices = terrainGeometry.indices();
    }

    void createVertexBuffer() {
        VkDeviceSize bufferSize = sizeof(NwVertex) * mVertices.size();
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.proj = glm::perspectiveRH_ZO(45.0f, mSwapchainExtent.width / static_cast<float>(mSwapchainExtent.height),
                                         0.1f, 1000.0f);
        ubo.proj[1][1] *= -1;

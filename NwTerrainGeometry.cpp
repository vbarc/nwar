#include "NwTerrainGeometry.h"

#include <algorithm>

#include "NwBicubicInterpolation.h"
...
static vec3 vertexNormal(const std::vector<NwVertex>& vertices, int index1, int index2, int index3);

NwTerrainGeometry::NwTerrainGeometry() {
    double startTime = nwGetTime();

    NwDisplacementMap dm("terrain-map.png");

    std::vector<std::vector<NwBicubicInterpolation>> interpolations;
    ...
    for (int j = 1; j < dm.depth() - 2; j++) {
        ...
        for (int i = 1; i < dm.width() - 2; i++) {
            ...
            interpolations[j][i] = NwBicubicInterpolation(i, j, f);
        }
    }
    ...
    NGL_LOGI("Terrain geometry generation time: %0.3fs", nwGetTime() - startTime);
}

NwTerrainGeometry::~NwTerrainGeometry() {}

int NwTerrainGeometry::width() const {
    return kGranularity + 1;
}

int NwTerrainGeometry::depth() const {
    return kGranularity + 1;
}

const std::vector<NwVertex>& NwTerrainGeometry::vertices() const {
    return mVertices;
}

const std::vector<uint32_t>& NwTerrainGeometry::indices() const {
    return mIndices;
}

const std::vector<float>& NwTerrainGeometry::heights() const {
    return mHeights;
}
...
vec3 vertexNormal(const std::vector<NwVertex>& vertices, int index1, int index2, int index3) {

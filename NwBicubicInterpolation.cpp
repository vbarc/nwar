...
NwBicubicInterpolation::NwBicubicInterpolation() {}

NwBicubicInterpolation::NwBicubicInterpolation(int column, int row, float samples[16]) : mColumn(column), mRow(row) {
    F f(samples);
    glm::mat4 m = glm::mat4(                                                                                          //
...
float NwBicubicInterpolation::interpolate(float x, float y) {
    float x2 = x * x;
    float x3 = x2 * x;
    float y2 = y * y;
    float y3 = y2 * y;
